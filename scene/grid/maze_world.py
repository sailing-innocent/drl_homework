...
from .maze import generate_maze, print_maze, get_maze_debug
...
class MazeWorld(gym.Env):
    ...
    def __init__(self, 
        ...
        self.maze = generate_maze(gen_mode, size, size, startnode, endnode)
        self.size = size
        ...
        # we have four actions: up, right, down, left
        self.action_space = spaces.Discrete(4)
        self._action_to_code = {
            0: 0b1000,
            1: 0b0100,
            2: 0b0010,
            3: 0b0001
    ...
    def reset(self, seed=None, options=None):
        ...
        self._agent_location = np.array([self.startnode[0], self.startnode[1]])
        self._target_location = np.array([self.endnode[0], self.endnode[1]])
    ...
    def step(self, action):
        # encoding action
        code = self._action_to_code[action]
        if code & self.maze[self._agent_location[0]][self._agent_location[1]] != 0:
            # hit the wall
            direction = np.array([0,0]) # dont move
        else:
    ...
    def _render_frame(self):
        ...
        pygame.draw.circle(
            canvas,
            (0, 0, 255),
            pix_square_size * (self._agent_location[::-1] + 0.5),
            pix_square_size // 3,
        )
        ...
        for i in range(self.size):
            for j in range(self.size):
                # if the first row and has upper bound
                if i == 0 and self.maze[i][j] & 0b1000 != 0:
                    pygame.draw.line(
                        canvas, # surface
                        (0, 0, 0), # color
                        (j * pix_square_size, 0),
                        ((j+1)*pix_square_size, 0),
                        width=3
                    )
                # if the first row and has left bound
                if j == 0 and self.maze[i][j] & 0b0001 != 0:
                    pygame.draw.line(
                        canvas, # surface
                        (0, 0, 0), # color
                        (0, i * pix_square_size),
                        (0, (i+1)*pix_square_size),
                        width=3
                    )

                # draw the lower bound 
                if self.maze[i][j] & 0b0010 != 0:
                    pygame.draw.line(
                        canvas, # surface
                        (0, 0, 0), # color
